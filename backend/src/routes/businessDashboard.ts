...
router.get('/business', authenticateToken, asyncHandler(async (req: Request, res: Response) => {
  ...
  // Check if user has business role or business name
  ...
  // Allow access if user has business role, admin role, OR has a business name
  if (userInfo.role !== 'business' && userInfo.role !== 'admin' && !userInfo.businessName) {
  ...
  try {
    ...
    const dashboardData = await safePrismaQuery(async (client) => {
      ...
      const [
        ...
        // Upcoming activities (next 7 days) - simplified query
      ...
      // Generate simplified finance data (mock data for now to speed up)
      const financeData = [
